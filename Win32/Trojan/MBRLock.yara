...
rule Trojan_Win32_MBRLock_MVA_2147902590_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/MBRLock.MVA!MTB"
        threat_id = "2147902590"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "MBRLock"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_2_1 = "Your disk have a lock" ascii //weight: 2
        $x_2_2 = "physicaldrive0" ascii //weight: 2
        $x_1_3 = "eyuyan" ascii //weight: 1
        $x_1_4 = "l.chs\\afxres.rc" ascii //weight: 1
    condition:
        (filesize < 20MB) and
        (
